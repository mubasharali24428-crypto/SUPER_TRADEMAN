...
def test_no_signal_when_price_within_band():
    closes = [
        100.0
    ] * 130  # flat throughout -> zero std in the z-score lookback -> no signal
...
def test_long_signal_fires_when_trend_is_flat():
    ...
    signal = generate_signal(
        _candles(closes), "BTC/USDT", NOW, window=20, z_threshold=2.0
    )
...
def test_long_signal_suppressed_during_confirmed_downtrend():
    ...
    assert (
        generate_signal(_candles(closes), "BTC/USDT", NOW, window=20, z_threshold=2.0)
        is None
    )
