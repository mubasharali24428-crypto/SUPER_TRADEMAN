...
def test_micro_alpha_engine_captures_post_crash_dislocation():
    ...
    sig = alpha.calculate_signal(
        current_micro_price=95.0, stress_score=0.05, sigma=0.01
    )
    assert sig.is_valid
...
def test_sniper_dislocation_trigger():
    ...
    at_boundary = alpha.calculate_signal(
        current_micro_price=97.0, stress_score=0.05, sigma=0.01
    )
    assert not at_boundary.is_valid  # exactly -3.0% dislocation does not trigger

    above_boundary = alpha.calculate_signal(
        current_micro_price=96.9, stress_score=0.05, sigma=0.01
    )
