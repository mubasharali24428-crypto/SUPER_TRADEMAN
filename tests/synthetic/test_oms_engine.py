...
from trading.synthetic.oms_engine import (AtomicMailbox, ExchangeMessage,
                                          OMSActorEngine, OMSEventTier,
                                          OrderStatus)
...
def test_pending_cancel_catches_late_fill():
    ...
    fill_msg = ExchangeMessage(
        seq_num=1, order_id="ord_cancel_target", msg_type="FILL", qty=1.0, price=50000.0
    )
    ...
    rej_msg = ExchangeMessage(
        seq_num=2,
        order_id="ord_cancel_target",
        msg_type="CANCEL_REJECTED",
        qty=0.0,
        price=0.0,
    )
...
def test_sequence_number_supremacy():
    ...
    msg2 = ExchangeMessage(
        seq_num=2, order_id="ord_seq", msg_type="CANCEL_REJECTED", qty=0.0, price=0.0
    )
    msg1 = ExchangeMessage(
        seq_num=1, order_id="ord_seq", msg_type="FILL", qty=1.0, price=50000.0
    )
...
def test_actor_sequential_processing():
    ...
    engine.enqueue_event(
        OMSEventTier.TIER_2_EXECUTION,
        "SNIPER_ENTRY",
        {"order_id": "o1", "side": "BUY", "price": 100.0, "qty": 1.0},
    )
    engine.enqueue_event(
        OMSEventTier.TIER_2_EXECUTION,
        "SNIPER_ENTRY",
        {"order_id": "o2", "side": "BUY", "price": 100.0, "qty": 1.0},
    )
...
def test_priority_queue_ordering():
    ...
    engine.enqueue_event(
        OMSEventTier.TIER_2_EXECUTION,
        "SNIPER_ENTRY",
        {"order_id": "t2", "side": "BUY", "price": 100.0, "qty": 1.0},
    )
    engine.enqueue_event(OMSEventTier.TIER_1_STATE, "REGIME_SHIFT", {})
    engine.enqueue_event(
        OMSEventTier.TIER_0_SAFETY, "KILL_SWITCH", {"order_id": "nonexistent"}
    )
...
def test_tier0_preempts_tier2():
    ...
    engine.enqueue_event(
        OMSEventTier.TIER_2_EXECUTION,
        "SNIPER_ENTRY",
        {"order_id": "sniper1", "side": "BUY", "price": 100.0, "qty": 1.0},
    )
    engine.enqueue_event(
        OMSEventTier.TIER_0_SAFETY, "STALE_QUOTE_DETECTED", {"order_id": "stale1"}
    )

    first = engine.run_event_step()
    assert (
        first.event_type == "STALE_QUOTE_DETECTED"
    )  # Tier 0, queued second, still processed first
...
def test_pending_cancel_state_transition():
    ...
    assert (
        engine.orders["ord1"]["status"] == OrderStatus.PENDING_CANCEL
    )  # immediate, no queueing needed
...
def test_pending_cancel_memory_retention():
    ...
    fill = ExchangeMessage(
        seq_num=1, order_id="ord1", msg_type="FILL", qty=1.0, price=100.0
    )
...
def test_sequence_number_supremacy_fill_first():
    ...
    reject_msg = ExchangeMessage(
        seq_num=102, order_id="ord1", msg_type="CANCEL_REJECTED", qty=0.0, price=0.0
    )
    fill_msg = ExchangeMessage(
        seq_num=101, order_id="ord1", msg_type="FILL", qty=1.0, price=100.0
    )
...
def test_sequence_number_supremacy_reject_first():
    ...
    fill_msg = ExchangeMessage(
        seq_num=102, order_id="ord2", msg_type="FILL", qty=1.0, price=100.0
    )
    reject_msg = ExchangeMessage(
        seq_num=101, order_id="ord2", msg_type="CANCEL_REJECTED", qty=0.0, price=0.0
    )
...
def test_late_fill_on_pending_cancel():
    ...
    late_fill = ExchangeMessage(
        seq_num=1, order_id="ord1", msg_type="FILL", qty=2.0, price=100.0
    )
...
def test_late_rejection_on_filled_order():
    ...
    late_reject = ExchangeMessage(
        seq_num=2, order_id="ord1", msg_type="CANCEL_REJECTED", qty=0.0, price=0.0
    )
...
def test_ring_buffer_backpressure():
    ...
    for i in range(200):
        engine.enqueue_event(
            OMSEventTier.TIER_2_EXECUTION,
            "SNIPER_ENTRY",
            {"order_id": f"t2_{i}", "side": "BUY", "price": 100.0, "qty": 0.1},
        )
    ...
    assert (
        len(engine.event_pq) == 205
    )  # nothing dropped on enqueue, unbounded by design
...
def test_oms_exchange_reject_handling():
    ...
    engine.submit_order(
        "ord1", "BUY", -1.0, 1.0
    )  # invalid price, simulating what the exchange would reject

    reject_msg = ExchangeMessage(
        seq_num=1, order_id="ord1", msg_type="REJECT", qty=0.0, price=0.0
    )
...
def test_oms_graceful_shutdown():
    ...
    for i in range(10):
        engine.enqueue_event(
            OMSEventTier.TIER_2_EXECUTION,
            "SNIPER_ENTRY",
            {"order_id": f"ord_{i}", "side": "BUY", "price": 100.0, "qty": 1.0},
        )
    ...
    assert (
        len(engine.orders) == 10
    )  # every SNIPER_ENTRY was actually applied, not skipped
...
def test_safe_boundary_drain():
    ...
    engine.mailbox.put_state_sync(
        {"exchange_position": 1.02}
    )  # small 2% discrepancy -> DELTA_SYNC, not the breaker
...
def test_delta_calculation_accuracy():
    engine = OMSActorEngine(
        max_discrepancy_pct=0.50
    )  # wide enough to isolate the delta math from the breaker
    engine.local_position = 400.0

    engine.mailbox.put_state_sync(
        {"exchange_position": 500.0}
    )  # +100 share discrepancy
    engine.run_event_step()
...
def test_delta_sync_successful_reconciliation():
    ...
    assert engine.local_position == pytest.approx(
        52.0
    )  # local now matches exchange exactly
...
def test_halt_circuit_breaker_stops_generation():
    ...
    engine.local_position = 100.0
    engine.mailbox.put_state_sync(
        {"exchange_position": 150.0}
    )  # 50% discrepancy -> trips breaker
    ...
    engine.enqueue_event(
        OMSEventTier.TIER_2_EXECUTION,
        "SNIPER_ENTRY",
        {"order_id": "blocked_ord", "side": "BUY", "price": 100.0, "qty": 1.0},
    )
    engine.run_event_step()
    assert (
        "blocked_ord" not in engine.orders
    )  # new order generation refused while halted
    ...
    engine.enqueue_event(
        OMSEventTier.TIER_0_SAFETY, "EMERGENCY_CANCEL", {"order_id": "existing_ord"}
    )
    engine.run_event_step()
...
def test_reconciler_api_latency_tolerance():
    ...
    engine.enqueue_event(
        OMSEventTier.TIER_2_EXECUTION,
        "SNIPER_ENTRY",
        {"order_id": "noop", "side": "BUY", "price": 100.0, "qty": 0.0},
    )
    engine.run_event_step()
