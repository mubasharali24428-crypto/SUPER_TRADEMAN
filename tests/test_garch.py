...
def test_garch_fit_forecast_on_synthetic_data(rng):
    # Generate 150 candles of log returns with alternating volatility
    returns = list(rng.normal(0, 0.02, 150))
    returns[50:100] = rng.normal(0, 0.06, 50)  # Volatility spike
...
def test_garch_high_volatility_detection(rng):
    # Generate extreme volatility return series (~100% annual vol)
    high_vol_returns = list(rng.normal(0, 0.08, 100))
