...
def generate_trending_candles(num: int, start_price: float = 100.0, trend_drift: float = 0.0008, seed: int = 42):
...
def run_benchmark_suite(candles_btc: list, candles_eth: list, candles_sol: list):
    ...
    configs = [
        ...
        },
        {
            ...
            "risk_cfg": RiskConfig(risk_pct=0.005, max_drawdown=0.30, max_heat=0.10, max_heat_high_vol=0.08, correlation_threshold=0.5, max_positions_per_asset_class=10),
            "strategy": make_momentum_strategy(target_mult=0.03, stop_pct=0.01),
            "bt_cfg": BacktestConfig(),
            "account": AccountState(equity=10000.0, peak_equity=10000.0),
            "candles": candles_btc,
        ...
        },
        {
            ...
            "risk_cfg": RiskConfig(risk_pct=0.005, max_drawdown=0.40, max_heat=0.10, max_heat_high_vol=0.08, correlation_threshold=0.5, max_positions_per_asset_class=10),
            "strategy": make_momentum_strategy(target_mult=0.04, stop_pct=0.01),
            "bt_cfg": BacktestConfig().with_trailing_atr(mult=2.0, period=14),
            "account": AccountState(equity=10000.0, peak_equity=10000.0),
            "candles": candles_btc,
    ...
    t0 = time.perf_counter()
    shared_risk_cfg = RiskConfig(risk_pct=0.005, max_drawdown=0.40, max_heat=0.10, max_heat_high_vol=0.08, correlation_threshold=0.5, max_positions_per_asset_class=10)
...
def format_table(results):
    lines = []
    lines.append(f"| {'Configuration':<45} | {'Trades':<6} | {'Win Rate':<8} | {'Bayes WR':<8} | {'Avg R':<6} | {'Total Return':<13} | {'Max DD':<6} | {'Sharpe':<6} | {'Sortino':<7} | {'Tail-VaR':<8} | {'Time (ms)':<9} |")
    lines.append(f"|{'-'*47}|{'-'*8}|{'-'*10}|{'-'*10}|{'-'*8}|{'-'*15}|{'-'*8}|{'-'*8}|{'-'*9}|{'-'*10}|{'-'*11}|")
    for r in results:
        tot_ret_str = f"{r['total_return_pct']*100:,.1f}%" if abs(r['total_return_pct']) < 1e6 else f"{r['total_return_pct']:.2e}%"
        lines.append(f"| {r['name']:<45} | {r['trades']:<6} | {r['win_rate']:>7.1%} | {r['bayesian_wr']:>7.1%} | {r['avg_r']:>6.2f} | {tot_ret_str:>13} | {r['max_dd_pct']:>6.1%} | {r['sharpe']:>6.3f} | {r['sortino']:>7.3f} | {r['tail_var_99']*100:>7.2f}% | {r['runtime_ms']:>9.1f} |")
    return chr(10).join(lines)

def main():
    print("=" * 110)
    print("ALGO-TRADING SYSTEM: EXTENDED PERFORMANCE BENCHMARK WITH SORTINO, TAIL-VAR, & BAYESIAN STATS")
    print("=" * 110)

    # 1. Trending / Momentum Market (Positive drift)
    candles_trend_btc = generate_trending_candles(2000, start_price=100.0, trend_drift=0.0008, seed=42)
    candles_trend_eth = generate_trending_candles(2000, start_price=2000.0, trend_drift=0.0008, seed=99)
    candles_trend_sol = generate_trending_candles(2000, start_price=20.0, trend_drift=0.0008, seed=123)
    res_trend = run_benchmark_suite(candles_trend_btc, candles_trend_eth, candles_trend_sol)
    ...
    for strat, info in allocator.summary().items():
        print(f" - {strat:<16}: Policy Probability = {info['policy_prob']:>6.1%}, Bayesian Win Rate = {info['bayesian_win_rate']:>6.1%}, Avg Reward = {info['avg_reward']:>5.2f}R")
