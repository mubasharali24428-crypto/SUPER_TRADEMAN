...
from trading.risk.models import (_ISSUER, AccountState, ApprovedExit,
                                 ApprovedOrder, Position, Side)
from trading.risk.portfolio_circuit_breaker import (CircuitBreakerTier,
...
class OperationalDrillHarness:
    ...
    async def run_drill_1_stale_data_rejection(self) -> DrillResultRecord:
        ...
        oms = OrderManagementSystem(
            venue_adapter=venue, staleness_sentinel=sentinel, execution_mode=self.mode
        )
        ...
        exit_order = ApprovedExit(
            asset="BTC", asset_class="crypto", reason="stale_drill_exit", issuer=_ISSUER
        )
    ...
    async def run_drill_2_portfolio_drawdown_tier2(self) -> DrillResultRecord:
        ...
        cb = PortfolioCircuitBreaker(
            ...
            entry_block_threshold_pct=0.05,
            flatten_threshold_pct=0.08,
        )
        ...
        tier, exits = cb.evaluate_portfolio_drawdown(
            account, mark_prices={"BTC": 44000.0}
        )
        passed = (tier == CircuitBreakerTier.ENTRY_BLOCK) and (
            not cb.is_entry_allowed()
        )
    ...
    async def run_drill_3_portfolio_flatten_tier3(self) -> DrillResultRecord:
        ...
        tier, exits = cb.evaluate_portfolio_drawdown(
            account, mark_prices={"BTC": 41000.0, "ETH": 2900.0}
        )
        passed = (
            (tier == CircuitBreakerTier.FLATTEN)
            and (len(exits) == 2)
            and all(e.issuer is _ISSUER for e in exits)
        )
    ...
    async def run_drill_6_partial_fill_finalization(self) -> DrillResultRecord:
        ...
        passed = (oms.active_orders[cid] == OrderState.PARTIAL_FILL_FINALIZED) and (
            dev_event.realized_qty == 0.4
        )
    ...
    async def run_drill_7_wash_trade_prevention(self) -> DrillResultRecord:
        ...
        oms = OrderManagementSystem(
            venue_adapter=venue, order_chaser=chaser, execution_mode=self.mode
        )
        ...
        passed = (
            chaser.working_orders["cid_long_drill"].status == OrderState.CANCELED
        ) and (state in (OrderState.SUBMITTED, OrderState.FILLED))
