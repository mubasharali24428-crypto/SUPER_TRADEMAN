...
class EVTRiskEngine:
    ...
    def estimate_tail_risk(
        ...
        try:
            ...
            scale_factor = float(
                np.clip(self.target_tail_var_cap / max(cvar_99, 1e-4), 0.20, 1.25)
            )
            ...
            logger.warning(
                "EVT GPD fitting failed (%s). Using empirical fallback.", exc
            )
            return self._fallback_risk(arr, is_returns=is_returns)
    ...
    def _fallback_risk(self, arr: np.ndarray, is_returns: bool) -> EVTRiskResult:
        ...
        if len(losses) > 0:
            ...
            var_99 = (
                float(np.quantile(losses, 0.99)) if len(losses) >= 100 else var_95 * 1.3
            )
            cvar_99 = (
                float(np.mean(losses[losses >= var_95]))
                if any(losses >= var_95)
                else var_99 * 1.2
            )
            cvar_99 = max(cvar_99, var_99)
        else:
