...
def classify_ccxt_error(exc: BaseException) -> OrderState:
    ...
    """
    if isinstance(exc, ccxt.RateLimitExceeded):        # IS-A NetworkError in ccxt >=4; check FIRST
...
def next_state_on_venue_error(current_state: OrderState, exc: BaseException) -> OrderState:
...
class CcxtLiveAdapter(VenueAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        self.account_state = account_state if account_state is not None else AccountState(
        ...
        self.calls: list[Dict[str, Any]] = []  # audit trail of would-be/real submissions
    ...
    def _get_exchange(self):
        ...
        if self._exchange is None:
            ...
            self._exchange = klass({"apiKey": self._creds.get("apiKey"),
                                    "secret": self._creds.get("secret"),
                                    "password": self._creds.get("password"),
                                    "enableRateLimit": True})
            logger.info("built ccxt %s %s", self.exchange_id, getattr(ccxt, "__version__", "?"))
        return self._exchange

    def _check_kill_switch(self) -> None:
        if self.account_state.kill_switch:
            raise KillSwitchActiveError("kill switch tripped: submission blocked pre-submit")
        ...
        for attempt in range(1 + _MAX_RETRIES):
            try:
                ...
                logger.warning(
                    ...
                    method_name, attempt + 1, 1 + _MAX_RETRIES, exc, delay,
                )
                await asyncio.sleep(delay)
        assert last_exc is not None
        raise last_exc

    async def _quantized(self, symbol: str, quantity: float, price: Optional[float]) -> tuple[float, Optional[float]]:
    ...
    async def create_order(self, order: ApprovedOrder, client_order_id: str) -> Dict[str, Any]:
        ...
        try:
            resp = await self._call("create_order", symbol, "limit", side, qty, px, params)
        except ccxt.RateLimitExceeded:
            raise                                  # propagate untouched to caller
        except ccxt.BaseError as exc:
            logger.error("venue rejected %s: %s -> %s", client_order_id, exc, classify_ccxt_error(exc))
            raise
        return {
            "id": resp.get("id"),
    ...
    async def create_exit(self, exit_order: ApprovedExit, client_order_id: str) -> Dict[str, Any]:
        ...
        if self.dry_run:
            ...
            self.calls.append(entry)
            logger.warning("[DRY-RUN EXIT] would submit close for %s (%s)", symbol, exit_order.reason)
            return {
        ...
        try:
            resp = await self._call("create_order", symbol, "market", "sell", None, None, params)
        except ccxt.RateLimitExceeded:
            raise
        except ccxt.BaseError as exc:
            logger.error("exit failed %s: %s -> %s", client_order_id, exc, classify_ccxt_error(exc))
            raise
        return {
            "id": resp.get("id"),
            "clientOrderId": resp.get("clientOrderId", client_order_id),
    ...
    async def cancel_order(self, client_order_id: str, symbol: str) -> Dict[str, Any]:
        if self.dry_run:
            entry = {"client_order_id": client_order_id, "symbol": symbol, "op": "cancel", "dry_run": True}
            self.calls.append(entry)
            logger.warning("[DRY-RUN CANCEL] would cancel %s on %s", client_order_id, symbol)
            return {"clientOrderId": client_order_id, "status": "canceled", "dry_run": True}
    ...
    async def fetch_order(self, client_order_id: str, symbol: str) -> Optional[Dict[str, Any]]:
        if self.dry_run:
            entry = {"client_order_id": client_order_id, "symbol": symbol, "op": "fetch_order", "dry_run": True}
            self.calls.append(entry)
            logger.warning("[DRY-RUN FETCH_ORDER] %s", entry)
            return {"clientOrderId": client_order_id, "status": "dry_run", "dry_run": True}

        try:
            resp = await self._call(
                "fetch_order", client_order_id, symbol, params={"origClientOrderId": client_order_id}
            )
    ...
    async def fetch_open_orders(self, symbol: Optional[str] = None) -> list[Dict[str, Any]]:
        if self.dry_run:
            self.calls.append({"op": "fetch_open_orders", "symbol": symbol, "dry_run": True})
    ...
    async def get_instrument_info(self, symbol: str) -> InstrumentInfo:
        ...
        if isinstance(amount_prec, (int,)) and not isinstance(amount_prec, bool) and amount_prec > 0:
