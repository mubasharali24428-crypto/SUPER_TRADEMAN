...
class ChaosInjector:
    ...
    def inject_flash_crash(self, drop_pct: float = 0.15) -> float:
        ...
        logger.warning(
            f"[CHAOS_FLASH_CRASH] Injected flash crash: {old_price} -> {new_price} (-{drop_pct*100:.1f}%)"
        )
        return new_price

    def inject_liquidity_vacuum(
        self, widen_factor: float = 10.0
    ) -> Tuple[float, float]:
        ...
        self.lob.bids = [
            LimitOrder("vac_b", "buy", new_b, 1.0, time.time() * 1000, "chaos")
        ]
        self.lob.asks = [
            LimitOrder("vac_a", "sell", new_a, 1.0, time.time() * 1000, "chaos")
        ]
        logger.warning(
            f"[CHAOS_LIQUIDITY_VACUUM] Injected liquidity vacuum: spread widened {widen_factor}x ({new_b} - {new_a})"
        )
        return new_b, new_a
    ...
    def inject_data_staleness(self) -> float:
        ...
        logger.warning(
            f"[CHAOS_DATA_STALENESS] Simulated websocket data staleness of {stale_delay_sec}s."
        )
    ...
    def inject_gap_open(self, gap_pct: float = 0.20) -> float:
        ...
        logger.warning(
            f"[CHAOS_GAP_OPEN] Injected gap open: {old_p} -> {new_p} (+{gap_pct*100:.1f}%)"
        )
        return new_p
    ...
    def execute_spoof_and_dump_stage(self, stage: int) -> Dict[str, Any]:
        ...
        if stage == 1:
            ...
            b, a = self.lob.get_best_bid_ask()
            self.lob.bids.insert(
                0,
                LimitOrder(
                    "spoof_fake_buy", "buy", b, 100.0, time.time() * 1000, "spoofer"
                ),
            )
            ...
            self.lob.bids = [
                ord_obj for ord_obj in self.lob.bids if ord_obj.agent_id != "spoofer"
            ]
