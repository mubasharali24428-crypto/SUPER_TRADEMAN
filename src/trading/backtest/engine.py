...
class BacktestConfig:
    ...
    max_hold_bars: int = (
        24  # time stop: exit if the trade hasn't resolved within N bars
    )
    trail_atr_mult: float | None = (
        None  # None = fixed stop. Set to trail the stop by N x ATR.
    )
    trail_atr_period: int = 14

    def with_trailing_atr(
        self, mult: float = 2.0, period: int = 14
    ) -> "BacktestConfig":
...
def _apply_slippage(
    price: float, side: Side, entering: bool, config: BacktestConfig
) -> float:
    is_buy = (side is Side.LONG) == entering
    return (
        price * (1 + config.slippage_pct)
        if is_buy
        else price * (1 - config.slippage_pct)
    )


def _commission(
    fill_price: float, position_size: float, config: BacktestConfig
) -> float:
    return fill_price * position_size * config.commission_pct


def _check_exit(
    side: Side, stop_price: float, target_price: float, high: float, low: float
):
...
def _close_trade(open_trade, exit_price, exit_time, reason, asset, config) -> tuple:
    exit_fill = _apply_slippage(
        exit_price, open_trade["side"], entering=False, config=config
    )
    sign = 1 if open_trade["side"] is Side.LONG else -1
    raw_pnl = (
        sign * (exit_fill - open_trade["entry_fill"]) * open_trade["position_size"]
    )
    fees = _commission(
        open_trade["entry_fill"], open_trade["position_size"], config
    ) + _commission(exit_fill, open_trade["position_size"], config)
...
def _record_learning_graph(
    learning_graph: LearningGraph | None, asset: str, trade: Trade, signal
):
...
def run_backtest(
    ...
    breakeven_p: float
    | None = None,  # None = derive from realized payoffs; see _breakeven_win_rate
    ...
    for i, candle in enumerate(candles):
        ...
        if open_trade is not None:
            ...
            if exit_price is not None:
                ...
                trades.append(trade)
                _record_learning_graph(
                    learning_graph, asset, trade, open_trade["signal"]
                )
                open_trade = None
        ...
        if signal is not None:
            account_snapshot = replace(
                starting_account, equity=equity, peak_equity=peak_equity
            )
    ...
    if open_trade is not None:
        ...
        trade, net_pnl = _close_trade(
            open_trade, last_close, exit_time, "end_of_data", asset, config
        )
        equity += net_pnl
        trades.append(trade)
    ...
    report = _build_report(
        starting_account.equity, equity, equity_curve, trades, breakeven_p
    )
    return BacktestResult(report=report, trades=trades, equity_curve=equity_curve)


def _build_report(
    start_equity, final_equity, equity_curve, trades, breakeven_p=None
) -> BacktestReport:
...
def _binomial_sf(k_min: int, n: int, p: float) -> float:
    ...
    return sum(
        math.comb(n, k) * p**k * (1 - p) ** (n - k) for k in range(k_min, n + 1)
    )
...
def bootstrap_trade_returns(
    ...
    def pct(p):
        return terminal_returns[
            min(len(terminal_returns) - 1, int(p * len(terminal_returns)))
        ]
