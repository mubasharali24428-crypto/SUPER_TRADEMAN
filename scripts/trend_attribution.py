...
async def load(exchange, symbol):
    ...
    candles = await fetch_ohlcv_range(
        exchange, symbol, "1d", until - int(7 * 365.25 * 86400_000), until
    )
...
async def main():
    ...
    print(
        f"{'symbol':10s} {'split':6s} {'trades':>6} {'ret':>8} {'maxDD':>7} {'Sharpe':>8} "
    ...
    for sym in SYMBOLS:
        ...
        for name, window in zip(("train", "test"), split_train_test(candles, 0.7)):
            ...
            res = run_backtest(
                window, sym, generate_trend_signal, engine, account, config=cfg
            )
            ...
            print(
                f"{sym:10s} {name:6s} {res.report.num_trades:>6} "
    ...
    for name, trades in per_split.items():
        ...
        for label, subset in (("LONG ", longs), ("SHORT", shorts)):
            if subset:
                ...
                print(
                    f"  {label} n={len(subset):>4}  win {wins / len(subset):>5.1%}  "
                    f"avgR {statistics.mean(rs):>+6.3f}  totalR {sum(rs):>+7.1f}"
                )
        print(
            f"  bootstrap terminal return: p5 {boot['p5']:+.2%}  p50 {boot['p50']:+.2%}  "
            f"p95 {boot['p95']:+.2%}"
        )
