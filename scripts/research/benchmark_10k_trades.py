...
def generate_trending_candles(
    num: int, start_price: float = 100.0, trend_drift: float = 0.0005, seed: int = 42
):
...
def run_10k_benchmark():
    ...
    candles_trend_btc = generate_trending_candles(
        CANDLE_COUNT, start_price=100.0, trend_drift=0.0005, seed=42
    )
    candles_trend_eth = generate_trending_candles(
        CANDLE_COUNT, start_price=2000.0, trend_drift=0.0005, seed=99
    )
    candles_trend_sol = generate_trending_candles(
        CANDLE_COUNT, start_price=20.0, trend_drift=0.0005, seed=123
    )

    candles_rw_btc = generate_synthetic_candles(
        CANDLE_COUNT, start_price=100.0, seed=42
    )
    candles_rw_eth = generate_synthetic_candles(
        CANDLE_COUNT, start_price=2000.0, seed=99
    )
    candles_rw_sol = generate_synthetic_candles(
        CANDLE_COUNT, start_price=20.0, seed=123
    )
    ...
    def execute_suite(candles_btc, candles_eth, candles_sol, regime_name):
        ...
        shared_risk_cfg = RiskConfig(
            risk_pct=0.005, max_drawdown=0.50, max_heat=0.10, max_heat_high_vol=0.08
        )
        ...
        res_btc = run_backtest(
            candles_btc, "BTC/USDT", strat, shared_engine, acc, bt_cfg
        )
        acc_eth = replace(
            acc,
        ...
        avg_r = (
            sum(t.r_multiple for t in all_trades) / len(all_trades) if all_trades else 0
        )
    ...
    def print_table(results, title):
        print(f"\n### {title}")
        print(
            f"| {'Configuration':<45} | {'Trades':<8} | {'Win Rate':<8} | {'Bayes WR':<8} | {'Avg R':<6} | {'Total Return':<13} | {'Max DD':<6} | {'Sharpe':<6} | {'Sortino':<7} | {'Tail-VaR':<8} | {'Time (ms)':<9} |"
        )
        print(
            f"|{'-'*47}|{'-'*10}|{'-'*10}|{'-'*10}|{'-'*8}|{'-'*15}|{'-'*8}|{'-'*8}|{'-'*9}|{'-'*10}|{'-'*11}|"
        )
        for r in results:
            tot_ret_str = (
            ...
            print(
                f"| {r['name']:<45} | {r['trades']:<8,} | {r['win_rate']:>7.1%} | {r['bayesian_wr']:>7.1%} | {r['avg_r']:>6.2f} | {tot_ret_str:>13} | {r['max_dd_pct']:>6.1%} | {r['sharpe']:>6.3f} | {r['sortino']:>7.3f} | {r['tail_var_99']*100:>7.2f}% | {r['runtime_ms']:>9.1f} |"
            )

    trend_results = execute_suite(
        candles_trend_btc, candles_trend_eth, candles_trend_sol, "Trending / Momentum"
    )
    rw_results = execute_suite(
        candles_rw_btc, candles_rw_eth, candles_rw_sol, "Random Walk / Chop"
    )
    ...
    for step in range(1, 10001):
        ...
        if step in checkpoint_steps:
            ...
            print(
                f"| {step:<8,} | {tm_p:>21.2%} | {mr_p:>19.2%} | {bo_p:>19.2%} | {leading:<18} |"
            )
