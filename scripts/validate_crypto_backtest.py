...
from trading.backtest.engine import (bootstrap_trade_returns, run_backtest,
                                     split_train_test)
...
async def main():
    ...
    for label, window in [("TRAIN (in-sample)", train), ("TEST (out-of-sample)", test)]:
        ...
        result = run_backtest(
            window, "BTC/USDT", generate_signal, RiskEngine(), account
        )
        r = result.report
        ...
        print(
            f"  win_rate           {r.win_rate:.2%}  (95% CI {r.win_rate_ci_low:.1%}-{r.win_rate_ci_high:.1%})"
        )
        print(
            f"  win_rate_p_value   {r.win_rate_p_value:.3f}  (H0: win rate = 33.3% breakeven)"
        )
        ...
        if r.num_trades >= 5:
            ...
            returns = (
                np.diff(trades_arr) / trades_arr[:-1]
                if len(trades_arr) > 1
                else np.array([0.0])
            )
            ...
            dsr = deflated_sharpe_ratio(
                sharpe_observed=r.sharpe_ratio,
                n_trials=n_trials,
            ...
            print(
                f"  dsr (deflated sharpe) {dsr:.3f}  (n_trials={n_trials}, n_obs={n_obs})"
            )
